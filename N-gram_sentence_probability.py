...
class NgramProb:
    ...
    def GetUnigramProbabilities(self, unigram_data):
        # count the total instances of tokens, not include start and end tokens
    ...
    def GetSmoothedProbabilities(self, unigram_data, bigram_data):
        ...
        # recalculate prob. smoothed_bigram_prob: for non zero prob, smoothed_bigram_prob_zero: for zero prob. 
        smoothed_bigram_prob = {}
        smoothed_bigram_prob_zero = {}
        # get smoothed bigram probability
        for key, value in bigram_data.items():
            key_unigram = key.split(" ")[0]
            smoothed_bigram_prob[key] = math.log10((value + k)/(unigram_data[key_unigram] + k*(len(unigram_data))))
        for key, value in unigram_data.items():
            smoothed_bigram_prob_zero[key] = math.log10(k/(value + k*(len(unigram_data))))       
        return smoothed_bigram_prob, smoothed_bigram_prob_zero
...
class NgramSentenceProb:
    ...
    def GetBigramSentenceProbability(self, sentence, bigram_prob):
        ...
        # calculate sentence bigram probability using chain rule
    ...
    def GetSmoothedSentenceProbability(self, sentence, smoothed_bigram_prob, smoothed_bigram_prob_zero):
        # add start and ending tokens to sentence
        sentence = "<s> " + sentence.replace("\n", " </s>")
        tokens = sentence.split(" ")
        ...
        # calculate sentence smoothed bigram probability using chain rule
        ...
        for j in smoothed_sentence_bigram:
            if j in smoothed_bigram_prob.keys():
                ...
                # if not in the bigram prob. model, use the smoothed prob.
                smoothed_bigram_sentence_prob += smoothed_bigram_prob_zero[j.split(" ")[0]]
...
if __name__ == "__main__":
    ...
    smoothed_bigram_prob, smoothed_bigram_prob_zero = ngram_prob.GetSmoothedProbabilities(unigram_count, bigram_count)
    ...
    # calculate sentence N-gram probability for test data
    for sentence in test_data:
        ...
        if unigram_sentence_prob != 0:
            print("Unigrams: logprob(S) = ", "%.4f" % unigram_sentence_prob)
        else:
            # if prob equals to 0
            print("Unigrams: logprob(S) = undfined")
        bigram_sentence_prob = ngram_sentence_prob.GetBigramSentenceProbability(sentence, bigram_prob)
        if bigram_sentence_prob != 0:
            print("Bigrams: logprob(S) = ", "%.4f" % bigram_sentence_prob)
        else:
            # if prob equals to 0
            print("Bigrams: logprob(S) = undefined")
        smoothed_bigram_sentence_prob = ngram_sentence_prob.GetSmoothedSentenceProbability(sentence, smoothed_bigram_prob, smoothed_bigram_prob_zero)
        if smoothed_bigram_sentence_prob != 0:
            print("Smoothed Bigrams: logprob(S) = ", "%.4f" % smoothed_bigram_sentence_prob)
        else:
            # if prob equals to 0
